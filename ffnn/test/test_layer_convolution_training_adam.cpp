...
TEST(TestLayerConvolutionWithOptimizers, Static_Adam)
{
  using namespace ffnn::layer;
  using namespace ffnn::optimizer;

  using convolution::ColEmbedding;
  using Options = convolution::options<16, 16, 3, 4, 4, 8, 1, 1, ColEmbedding>;
  ...
  auto hidden = boost::make_shared<Hidden>(Hidden::Configuration()
                                           .setOptimizer(boost::make_shared<Optimizer>(1e1)));
...
TEST(TestLayerConvolutionWithOptimizers, Dynamic_Adam)
{
  using namespace ffnn::layer;
  using namespace ffnn::optimizer;
