...
/***********************************************************/
// Creates network workflow with one Activation hidden
...
/***********************************************************/
TEST(TestLayerActivationWithOptimizers, GradientDescent)
{
  // Layer-type alias
  using Layer  = ffnn::layer::Layer<float>;
  using Input  = ffnn::layer::Input<float>;
  using Hidden = ffnn::layer::Activation<float, ffnn::neuron::LeCunSigmoid>;
  ...
  // Create some data
  Hidden::InputVectorType target_data = Hidden::InputVectorType::Zero(DIM);
